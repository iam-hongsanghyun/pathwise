...
#: The component kinds a project can copy in — exactly the dispatch keys of
#: :func:`~pathwise.data.components.copy_component_into` (a "stream" is a commodity).
_CATALOG_KINDS = ("technology", "stream", "measure", "macc")


def _catalog_entries(
    lib_id: str, lib: ComponentLibrary, scope: str, kinds: tuple[str, ...]
) -> list[dict[str, Any]]:
    """Flatten a library's components of the requested ``kinds`` into copy-ready rows.

    Each row's ``kind`` + ``component_id`` feed :func:`copy_component_into` directly.
    """
            ...
            rows.append(
                {
                    "scope": scope,
                    "library_id": lib_id,
                    "component_id": t.technology_id,
                    "label": t.technology_id,
                    "kind": "technology",
                }
            )
    if "stream" in kinds:
            ...
            rows.append(
                {
                    "scope": scope,
                    "library_id": lib_id,
                    "component_id": c.commodity_id,
                    "label": c.commodity_id,
                    "kind": "stream",
                }
            )
    if "measure" in kinds:
            ...
            rows.append(
                {
                    "scope": scope,
                    "library_id": lib_id,
                    "component_id": m.measure_id,
                    "label": m.label or m.measure_id,
                    "kind": "measure",
                }
            )
    if "macc" in kinds:
            ...
            rows.append(
                {
                    "scope": scope,
                    "library_id": lib_id,
                    "component_id": g.macc_id,
                    "label": g.label or g.macc_id,
                    "kind": "macc",
                }
            )
    return rows
...
@router.get("/session/{session_id}/component-catalog")
def list_component_catalog(session_id: str, kind: str | None = None) -> list[dict[str, Any]]:
    ...
    Each row is ``{scope, library_id, component_id, label, kind}``; ``kind`` and
    ``component_id`` are copy-ready (they feed ``copy_component_into``). Omitting
    ``kind`` returns all four kinds.
    """
    if kind is not None and kind not in _CATALOG_KINDS:
        raise HTTPException(status_code=422, detail=f"invalid kind '{kind}'")
    kinds = (kind,) if kind else _CATALOG_KINDS
    out: list[dict[str, Any]] = []
    for f in sorted(_components_dir().glob("*.sqlite")):
        try:
            lib = load_component_library(f)
        except Exception:  # a malformed file should not break the catalogue
            continue
        out.extend(_catalog_entries(f.stem, lib, "base", kinds))
    sl = _session_libs()
    for lib_id in sl.list_ids(session_id):
        slib = sl.get(session_id, lib_id)
        if slib is None:
            continue
        out.extend(_catalog_entries(lib_id, slib, "session", kinds))
    return out
