...
export const FIELD_META: Record<string, FieldMeta> = {
  ...
  // ── Flow / flow ───────────────────────────────────────────────────────
  flow_id: { info: "Unique id of the flow (flow)." },
  kind: { info: "energy · material · indirect · product · byproduct — what the flow represents." },
  unit: { info: "The physical unit the flow's quantities and prices are expressed in." },
  sector: { info: "The sector that PRODUCES this flow (electricity belongs to power, not steel). Blank = a general, industry-agnostic flow. Organisational only — the optimiser ignores it." },
  price: { info: "External purchase price of the flow.", unit: "currency / unit" },
  sale_price: { info: "Revenue when the flow is sold or exported.", unit: "currency / unit" },

  // ── IO row ────────────────────────────────────────────────────────────────────
  target: { info: "The flow consumed/produced (or the impact emitted) by this row." },
  role: { info: "input (consumed) · output (produced) · impact (emitted), per unit of throughput." },
  coefficient: { info: "Amount of the flow per unit of the technology's throughput.", unit: "flow unit / throughput" },
  coefficient_unit: { info: "Unit the coefficient is authored in. Blank = the target flow's own unit; a differing unit is converted to the flow's unit when the model is assembled." },
  ...
  share_min: { info: "Minimum share of the blend/slate group this flow must take.", unit: "fraction 0–1" },
  share_max: { info: "Maximum share of the blend/slate group this flow may take.", unit: "fraction 0–1" },
  ...
  amount: { info: "Quantity of the product this node must deliver to demand each year.", unit: "flow unit / yr" },
  max_purchase: { info: "Ceiling on how much of this flow may be bought externally per year (blank = no cap).", unit: "flow unit / yr" },
  available_from: { info: "First year this flow can be bought externally (blank = always).", unit: "year" },
  available_to: { info: "Last year this flow can be bought externally (blank = always).", unit: "year" },
};
