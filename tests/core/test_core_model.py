...
def test_single_process_cost_is_analytic() -> None:
    ...
    wb = {
        ...
        "commodities": [
            {"commodity_id": "gas", "kind": "energy", "unit": "MWh", "price": 10.0},
            {"commodity_id": "widget", "kind": "product", "unit": "u"},
        ],
        "technologies": [{"technology_id": "T", "opex": 0.0}],
        "processes": [
            {"process_id": "P", "company": "C", "baseline_technology": "T", "capacity": 100.0}
        ],
        "process_inputs": [{"technology_id": "T", "commodity_id": "gas", "intensity": 2.0}],
        "process_outputs": [
            {"technology_id": "T", "commodity_id": "widget", "yield": 1.0, "is_product": True}
        ],
        "demand": [{"company": "C", "commodity_id": "widget", "year": 2025, "amount": 50.0}],
    }
    res = _solve(wb)
...
def test_network_flow_routes_intermediate() -> None:
    ...
    wb = {
        "periods": [{"year": 2025, "duration_years": 1}],
        "commodities": [
            {"commodity_id": "gas", "kind": "energy", "price": 5.0},
            {"commodity_id": "mid", "kind": "material", "sellable": False},
            {"commodity_id": "prod", "kind": "product"},
        ],
        ...
        "process_inputs": [
            {"technology_id": "A", "commodity_id": "gas", "intensity": 3.0},
            {"technology_id": "B", "commodity_id": "mid", "intensity": 1.0},
        ],
        "process_outputs": [
            {"technology_id": "A", "commodity_id": "mid", "yield": 1.0},
            {"technology_id": "B", "commodity_id": "prod", "yield": 1.0, "is_product": True},
        ],
        "edges": [{"from_process": "F1", "to_process": "F2", "commodity_id": "mid"}],
        "demand": [{"company": "C", "commodity_id": "prod", "year": 2025, "amount": 40.0}],
    }
    res = _solve(wb)
    assert res["status"] == "optimal"
    # 40 prod ← 40 mid ← 120 gas × $5 = $600.
    np.testing.assert_allclose(res["objective"], 600.0, rtol=1e-6)
    flows = {(f["from"], f["to"], f["commodity"]): f["value"] for f in res["outputs"]["flows"]}
...
def test_carbon_price_makes_emissions_show() -> None:
    wb = {
        "periods": [{"year": 2025, "duration_years": 1}],
        "commodities": [
            {"commodity_id": "coal", "kind": "energy", "price": 1.0},
        ...
        "commodity_impacts": [{"commodity_id": "coal", "impact_id": "CO2", "factor": 2.0}],
        "impact_prices": [{"impact_id": "CO2", "year": 2025, "price": 7.0}],
        "demand": [{"company": "C", "commodity_id": "p", "year": 2025, "amount": 10.0}],
    }
    res = _solve(wb)
    assert res["status"] == "optimal"
