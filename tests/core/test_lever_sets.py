...
def _wb() -> dict:
    ...
    facilities receive their own copy. The lever cuts fuel use 20% for an
...
def test_set_linked_to_technology_expands_per_facility() -> None:
    ...
    assert {m.applies_to for m in prob.levers} == {"Big", "Small"}


def test_each_facility_adopts_independently() -> None:
    res = _solve(_wb())
    assert res["status"] == "optimal"
    adopted = {m["process"]: m["adoption"] for m in res["outputs"]["levers"]}
...
def test_direct_facility_link_still_works() -> None:
    wb = _wb()
    wb["lever_links"] = [{"set": "T retrofits", "applies_to": "Small"}]
    sc = ScenarioConfig.from_dict({"economics": {"base_year": 2025, "discount_rate": 0.0}})
    prob = assemble_problem(wb, sc)
    assert [m.applies_to for m in prob.levers] == ["Small"]


def test_negative_cost_block_is_adopted_immediately() -> None:
    wb = _wb()
    wb["lever_blocks"][0]["capex"] = -50.0  # e.g. a subsidised retrofit
    res = _solve(wb)
    adopted = {m["process"]: m["adoption"] for m in res["outputs"]["levers"]}
