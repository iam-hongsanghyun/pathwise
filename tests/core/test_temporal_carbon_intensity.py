"""Temporal carbon intensity: a commodity's impact factor may vary by year.

``commodity_impacts`` is the static factor; an optional long-format
``commodity_impacts_t`` (commodity_id, impact_id, year, factor) overrides it per
...
def _wb() -> dict:
    return {
        ...
        "commodities": [
            {"commodity_id": "fuel", "kind": "energy", "price": 1.0},
            {"commodity_id": "prod", "kind": "product"},
        ],
        "impacts": [{"impact_id": "CO2", "unit": "tCO2"}],
        "commodity_impacts": [{"commodity_id": "fuel", "impact_id": "CO2", "factor": 1.0}],
        # Year-varying override: 1.0 tCO2/unit in 2025 rising to 3.0 in 2030.
        "commodity_impacts_t": [
            {"commodity_id": "fuel", "impact_id": "CO2", "year": 2025, "factor": 1.0},
            {"commodity_id": "fuel", "impact_id": "CO2", "year": 2030, "factor": 3.0},
        ],
        ...
        "demand": [
            {"company": "C", "commodity_id": "prod", "year": y, "amount": 100} for y in (2025, 2030)
        ],
    }
...
def test_static_factor_is_the_fallback_when_no_year_given() -> None:
    wb = _wb()
    wb.pop("commodity_impacts_t")  # only the static 1.0 remains
